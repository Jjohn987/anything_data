...
Of course, the first key question was, **from where can I get the data?**  

![](/img/ctextscreen.png)

One website with a rich amount of Chinese text data regarding the classics is [ctext.org](https://ctext.org/). But I wondered "How can I get this in R?" Scraping wasn't entirely feasible due to the terms outlawing this practice. Secondly, scraping is a bit of a delicate operation - if text isn't formated uniformly across pages then you might be in for a headache. You also don't want to give the server unnecessary stress. 

The solution was to build an API. There is an API avaialble on ctext.org's website, but made in Python. I've never built an API or proto-API functions before, but the latter was easier than I thought. I'll save that for a future post.  

To the point - Many of the key functions revolve around passing a book or chapter as the args. So, it turned out scraping (keeping it limited and ethical) was a necessary evil. 

Without ado, here is the (very limited) scraping I did to create a book list, with chapters, which I put to use in my homemade API. 
