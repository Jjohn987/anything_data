...
In following similar pace with Tidytext, I first download my data. Here I use my package ```ctextclassics``` and the function ```get_books(c(...))``` but I want to point out the API limit is very low and I had to download my books between two different days. So therefore in this post I source my data in from a file of my text data.
...
If you run into the API limit, it's not that bad. Simply figure out which chapters produced NA and re-download them the next day with ```get_chapters("book", chapter:chapter)```. Before binding those rows with those that worked in the original dataframe, remember to filter out the NAs first.

Now, to get started - With any text analysis, tokenizing the text and filtering out stop words is fundamental. Tidytext segments English quite naturally, considering words are easily separated by spaces. However, I'm not so sure how it performs with Chinese characters.
...
We can see that the Analects has prevalant usage of words such as "Benevolence" (仁), "Gentleman" (君子) and "Confucius" (孔子). In fact, the Analects often mentions these terms - one examplary sentence may be:
...
The Mozi seems to have terms that are mostly civic related, such as country (国), citizen (民) and govern （治). 
 ...
 To conclude this post I summarize that I...
...
Overall, we saw the top 10 most common characters/words in these three ancient classical philosophical texts.

##Next Post 

In my next post, I would like to either **follow the same procedure but with bigrams**, and/or **apply LDA (Latent Dirichlet Allocation)** to these 3 works and see whether or not their chapters can be distinguished from one another. The word frequencies feel quite characteristic for these texts. But I'm not so sure that all of these chapters can be distinguished from one another (espcially because there are many shared words - Dao isn't solely mentioned in Taoist texts, the Analects also make many mentions of civic related concepts, and the Mozi is a long text written by different authors...) 
